...
#每个VGG模型每层通道数的配置，VGG后面数字代表有多少个卷积层
cfg = {
    'VGG11': [
        [64, 'M'],
        [128, 'M'],
...
class VGG(nn.Module):
    def __init__(self, vgg_name, num_classes, dropout):
        ...
        self.init_channels = 3 #初始化图像输入是三个通道
        ...
        if num_classes == 1000:
            self.classifier = nn.Sequential(
                nn.Flatten(),#展开，将张量拉成一维的向量
                nn.Linear(512*7*7, 4096),#全连接层
                nn.ReLU(inplace=True),#激活函数
                nn.Dropout(dropout),#Dropout可以比较有效的缓解过拟合的发生，在一定程度上达到正则化的效果。
                nn.Linear(4096, 4096),
    ...
    def _make_layers(self, cfg, dropout):
        ...
        for x in cfg:
            if x == 'M':
                ...
                layers.append(nn.Conv2d(self.init_channels, x, kernel_size=3, padding=1))#卷积层
                layers.append(nn.BatchNorm2d(x))#批量标准化处理
                layers.append(nn.ReLU(inplace=True))
                layers.append(nn.Dropout(dropout))
                self.init_channels = x
        return nn.Sequential(*layers)

    def forward(self, x):#由输入到输出的运行
        out = self.layer1(x)
        out = self.layer2(out)
        out = self.layer3(out)
        out = self.layer4(out)
